LootJS.lootTables(e => {
    ...
    let table = assembleVillageLoot(e, e.getLootTable('minecraft:chests/village/village_plains_house'),
        'plains',
        ...
        [[`${fd}:canvas`, 8], [`supplementaries:rope`, 8], [`${mc}:lead`, 3], [`${mc}:egg`, 4], [`${fd}:milk_bottle`, 2]],
        ['green', 'blue', 'brown', 'light_blue'],
        [[`${af}:villager_hat`, 2], 
         [`${af}:lucky_scarf`, 1],
         [`${af}:running_shoes`, 1]],
        [[`${mc}:stone_hoe`, 4], [`${mc}:fishing_rod`, 1], [`basicweapons:stone_quarterstaff`, 1]]
    )
