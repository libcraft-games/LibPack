LootJS.lootTables(e => {
    ...
    let table = assembleVillageLoot(e, e.getLootTable('minecraft:chests/village/village_taiga_house'),
        ...
        [[`${mc}:sweet_berries`,    6], 
         ...
         [`${mc}:spruce_sapling`, 5]],
        [[`${mc}:baked_potato`, 6], [`${mc}:cooked_mutton`, 3], [`${vd}:turnip_mutton_skewer`, 3], [`${vd}:sweet_potato_pie`, 1]],
        [[`${mc}:spruce_log`, 5], [`${mc}:iron_nugget`, 5], [`${xd}:sweet_berry_jelly`, 4], [`${mc}:spruce_boat`, 1]],
        ['green', 'brown', 'gray'],
