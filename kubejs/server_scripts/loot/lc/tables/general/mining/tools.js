LootJS.lootTables(e => {
    ...
    e.create('lc:tables/general/mining/tools').createPool(pool => {
        ...
        pool.addEntry(LootEntry.of(`${ie}:pickaxe_steel`)     .withWeight(5)  .withQuality(2) .damage([0.15, 0.35]))
        pool.addEntry(LootEntry.of(`${ie}:shovel_steel`)      .withWeight(3)  .withQuality(2) .damage([0.15, 0.35]))
        pool.addEntry(LootEntry.of(`${bw}:steel_dagger`)      .withWeight(2)  .withQuality(1) .damage([0.15, 0.55]))
