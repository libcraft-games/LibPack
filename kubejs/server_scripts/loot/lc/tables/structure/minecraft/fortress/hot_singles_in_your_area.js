...
LootJS.lootTables(e => {
    ...
    table.createPool(pool => {
        pool.addEntry(LootEntry.of(`${mc}:flint_and_steel`).damage([0.9, 1]).withWeight(32))
        pool.addEntry(LootEntry.of(`${cd}:blaze_upgrade_smithing_template`).withWeight(30))
        pool.addEntry(LootEntry.of(`${mc}:netherite_upgrade_smithing_template`).withWeight(28))
        pool.addEntry(LootEntry.of(`${mc}:rib_armor_trim_smithing_template`).withWeight(24))
        pool.addEntry(LootEntry.of(`${mc}:diamond_horse_armor`).withWeight(16))
        pool.addEntry(LootEntry.of(`${mc}:music_disc_11`).withWeight(16))
        pool.addEntry(LootEntry.of(`${mc}:book`).withWeight(12).enchantRandomly(`${ns}:wither_coated`))
        pool.addEntry(LootEntry.of(`${mc}:book`).withWeight(8).enchantRandomly(`${ns}:antidote`))
        pool.addEntry(LootEntry.of(`${mc}:book`).withWeight(7).enchantWithLevels([25, 35]))
